        ...
        while (!command.equals("Yohoho!")) {
            ...
            switch (commandParts[0]) {
                ...
                case "Drop":
                    ...
                    break;
                case "Steal":
                    int numberOfStealingItem = Integer.parseInt(commandParts[1]);
                    if (numberOfStealingItem >= 0 && numberOfStealingItem < treasureChest.length) {
                        for (int i = 0; i < numberOfStealingItem; i++) {
                            System.out.print(treasureChest[treasureChest.length - numberOfStealingItem + i]);
                            if (i != numberOfStealingItem - 1) {
                                System.out.print(", ");
                            }
                        }
                        String[] tempChest = new String[treasureChest.length - numberOfStealingItem];
                        for (int i = 0; i < tempChest.length; i++) {
                            tempChest[i] = treasureChest[i];
                        }
                        treasureChest = tempChest;

                    } else if (numberOfStealingItem >= 0) {
